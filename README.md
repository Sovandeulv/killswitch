...
Killswitch is *not a silver bullet*. Blocked players' actions still propagate through the mesh, just at higher latency. The mesh is what makes blocking work, and it's also what makes Killswitch resilient when the actual session host is the cheater: host detection passes over confirmed switchers and designates the busiest *clean* peer as our perceived host instead. The real host stays blocked, and gameplay state routes via the clean peers. This case turns out to be where blocking helps the most, a switching session host is the worst case because every player in the lobby routes session-level events through them, so taking ourselves out of that critical path is exactly what we want. We're leaning harder on the mesh than usual, but in practice that's fine as long as connections to other peers are healthy. Killswitch still helps in cheater-heavy lobbies as long as there are around 25% clean players in the session. If not, the best move is to find a different session, or better yet, a different game.
...
RDO/GTAO's mesh network is remarkably resilient. The game relays player information through the mesh, so even when you block a direct connection, that player's movements and actions still reach you through other peers. This adds latency to interactions with blocked players but keeps the session intact. The blocked player experiences the same latency on you, which is part of what makes blocking effective. Connections to clean players remain unaffected.

In practice, gameplay state propagates peer-to-peer through whichever direct links are healthy. As long as your console has paths to enough clean peers, you can play even when the link to the actual session host is degraded or blocked. Killswitch picks the busiest *clean* peer (not confirmed, not blocked) as our designated host and protects it from blocking — that guarantees at least one healthy direct link, and the mesh covers the rest. Sessions in this configuration are typically stable for extended play.
