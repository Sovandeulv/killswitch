...
Experienced cheaters use the switch tactically. They tap briefly, just enough that your hit packets arrive during the gap and get dropped. You land a clean headshot, and it does not register. Their shots land fine. When you catch them off guard, they die normally, but in a direct confrontation, they become effectively invincible. Should you manage to kill them between gaps, they ragdoll, and then a dead player on the ground shoots you. This frustrating post-mortem kill and teleporting are classic signs. 
...
Cronus Zen (and the older Cronus Max) is a hardware device that sits between the controller and console. It runs scripts that provide rapid fire, auto-headshot etc. In RDO, the telltale sign is a Carcano or repeater firing far faster than its intended rate, often combined with unnatural accuracy and jerky strafing movement. Cronus is common enough in RDO lobbies that most experienced players recognise the signs on sight. Killswitch cannot detect or do anything about Cronus. It operates entirely on the controller input side and never touches the network.

XIM devices are similar in spirit. They let players use a mouse and keyboard on console while the game still sees a standard controller, which means the console's built-in auto-aim stays active on top of the mouse precision. Some game studios now classify both XIM and Cronus as cheat devices, and have started injecting artificial input lag when spoofed controller input is detected. Rockstar added BattlEye to GTA Online on PC but never to RDO. On console there is no anti-cheat at all.

A newer class of cheats uses computer vision to provide AI-powered aim assist, running externally against the game's video output. This technology also works on console via a capture card routed to a nearby PC, feeding aim corrections back through a Cronus or XIM.

In addition to these, we haven’t even mentioned the numerous glitches in Rockstar games that can be exploited by cheaters.

None of these are problems Killswitch can solve. It operates at the network layer, and controller-side or vision-based cheats leave no trace in packet timing. They are worth knowing about so you can make an informed decision if it's worth playing PVP at all.

### How Killswitch Helps

Killswitch monitors packet flow between your router and console. When a player’s packets go silent and resume, the gap is classified and scored per IP. Crossing the threshold blocks the player. In **analysis mode**, it detects and logs without blocking, useful for studying sessions or verifying setups. In **operational mode**, it blocks the player.
...
The critical link is the session host. As long as your connection to the host stays up, the gameplay experience remains largely intact. You can block all other players except the session host and still play. Killswitch identifies the host during warmup and protects that IP from being blocked. Losing your connection to the host means losing the session. If the host migrates to a blocked player, the same thing happens. You see a network error and you're kicked back to the main menu. This is thankfully an infrequent occurrence.

If you become the session host (after host migration), blocking too many players may negatively impact others' experience, as they all depend on communicating with you.
